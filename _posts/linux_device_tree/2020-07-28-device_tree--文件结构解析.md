...
struct fdt_header {	
    ...
    fdt32_t boot_cpuid_phys;	 /* Which physical CPU id we're booting on */	/* version 3 fields below */	fdt32_t size_dt_strings;	 /* size of the strings block */ 	/* version 17 fields below */	
...
[![img](http://www.wowotech.net/content/uploadfile/201709/97111506232039.png?raw=true)](http://www.wowotech.net/content/uploadfile/201709/97111506232039.png?raw=true)

​	Device Tree中的节点信息举例如下图所示。

![img](http://www.wowotech.net/content/uploadfile/201709/1dc41506232049.png?raw=true)
...
[![img](http://www.wowotech.net/content/uploadfile/201709/42e01506232107.png?raw=true)](http://www.wowotech.net/content/uploadfile/201709/42e01506232107.png)
...
[![img](http://www.wowotech.net/content/uploadfile/201709/518c1506232119.png?raw=true)](http://www.wowotech.net/content/uploadfile/201709/518c1506232119.png)
...
[![img](http://www.wowotech.net/content/uploadfile/201709/52ae1506232173.png?raw=true)](http://www.wowotech.net/content/uploadfile/201709/52ae1506232173.png)
...
[![img](http://www.wowotech.net/content/uploadfile/201709/a40d1506232259.png?raw=true)](http://www.wowotech.net/content/uploadfile/201709/a40d1506232259.png)
...
{	
    unsigned long size;	
    int start;	
    void *mem;     /* 省略部分不重要部分 */	/* First pass, scan for size */	
    start = 0;	
    size = (unsigned long)unflatten_dt_node(blob, NULL, &start, NULL, NULL, 0, true);	
    size = ALIGN(size, 4); 	/* Allocate memory for the expanded device tree */	
    mem = dt_alloc(size + 4, __alignof__(struct device_node));	
    memset(mem, 0, size); 	/* Second pass, do actual unflattening */	
    start = 0;	
...
{	
    ...
    int offset;	
    int has_name = 0;	
    int new_format = 0; 	/* 获取node节点的name指针到pathp中 */	
    pathp = fdt_get_name(blob, *poffset, &l);	
    if (!pathp)		
        return mem; 	
    allocl = ++l; 	/* version 0x10 has a more compact unit name here instead of the full	 * path. we accumulate the full path size using "fpsize", we'll rebuild	 * it later. We detect this because the first character of the name is	 * not '/'.	 */	
    if ((*pathp) != '/') {		
        new_format = 1;		
        if (fpsize == 0) {			/* root node: special case. fpsize accounts for path* plus terminating zero. root node only has '/', so			 * fpsize should be 2, but we want to avoid the first			 * level nodes to have two '/' so we use fpsize 1 here			 */			
            fpsize = 1;			
            allocl = 2;			
            l = 1;			
            pathp = "";		
        } else {			/* account for '/' and path size minus terminal 0			 * already in 'l'			 */		fpsize += l;			
                allocl = fpsize;		
         }	
    } 	/* 分配struct device_node内存，包括路径全称大小 */	
    np = unflatten_dt_alloc(&mem, sizeof(struct device_node) + allocl,	__alignof__(struct device_node));	     if (!dryrun) {		
        char *fn;		
        of_node_init(np); 		/* 填充full_name，full_name指向该node节点的全路径名称字符串 */		
        np->full_name = fn = ((char *)np) + sizeof(*np);		
        if (new_format) {			/* rebuild full path for new format */			
            if (dad && dad->parent) {				
        ...
        memcpy(fn, pathp, l); 		/* 节点挂接到相应的父节点、子节点和姊妹节点 */		
...
[![img](http://www.wowotech.net/content/uploadfile/201709/395b1506232186.png?raw=true)](http://www.wowotech.net/content/uploadfile/201709/395b1506232186.png)
...
[![img](http://www.wowotech.net/content/uploadfile/201709/6d731506232188.png?raw=true)](http://www.wowotech.net/content/uploadfile/201709/6d731506232188.png)
